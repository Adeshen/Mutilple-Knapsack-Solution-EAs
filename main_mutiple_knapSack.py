...
class Main(object):
    def __init__(self,dataset="./TestDataset/f1_l-d_kp_10_269",num_iterations=1,algorithm=0,
        ...
        self.arr = ut.readfile_from_3(self.dataset)
        ...
        for pair in self.arr[1:]:
            c = 1
            print("origin data:",pair)
            v, w, k = pair[0],pair[1],pair[2]
            while k >= c:
                # k -= c
                if(k&c):
                    
                    self.weights.append(c * w)
                    self.values.append(c * v)
                c *= 2
            # self.weights.append(p * k)
            # self.values.append(h * k)
...
datasource='./TestDatasetMuti/'
filenames = ut.getListofFiles(datasource)

for f in filenames:
    try:
        print('Processing data set '+f)
        m = Main(dataset=datasource+f,num_iterations=300,algorithm=1)
        m.Run()
